...
class MultiAgentEnvironment:
    ...
    def reset(self):
        # --- CHANGE ---: Curriculum learning logic to make the task easier at the start
        ...
        self.initial_goals = self.goals.copy() # Keep a copy of the original goals
        ...
        self.last_dist_to_goal = np.linalg.norm(self.positions - self.goals, axis=1)
    ...
    def step(self, actions):
        # Store distances before moving for reward shaping
        last_dist_to_goal = np.linalg.norm(self.positions - self.goals, axis=1)
        ...
        reward = self.calculate_rewards(last_dist_to_goal, current_dist_to_goal)
        self.check_termination()
        
        return self.get_observations(), reward, self.done, {}
    
    def calculate_rewards(self, last_dist_to_goal, current_dist_to_goal):
        """Calculates a purely collective team reward to enforce coordination."""
        team_reward = 0.0
        ...
        # --- Reward Shaping ---
        progress = np.sum(last_dist_to_goal - current_dist_to_goal)
        team_reward += progress * 2.0
        
        # --- Collective Goal ---
        num_at_goal = np.sum(current_dist_to_goal < self.goal_radius)
        if num_at_goal == self.num_agents:
            team_reward += success_bonus
            self.done = True

        if np.any(self.time_remaining <= 0):
            team_reward += timeout_penalty
    ...
    def render(self, save_path=None):
        ...
        for i in range(self.num_agents):
            color = f'C{i}'
            # Use original goals for rendering to see where they should be
            ax.add_patch(plt.Circle(self.positions[i], self.collision_radius / 2, color=color, alpha=0.8))
            ax.add_patch(plt.Circle(self.initial_goals[i], self.goal_radius, color=color, fill=False, linestyle='--', alpha=0.8))
            ax.text(self.positions[i, 0], self.positions[i, 1], str(i), ha='center', va='center', color='white', fontsize=10)

        plt.title(f'Step: {self.step_count} | Episode: {self.current_episode}')
...
class MultiAgentTrainer:
    ...
    def __init__(self, env, num_episodes):
        ...
        # Create individual actors but a single shared critic and replay buffer
    ...
    def select_joint_action(self, states, graph_data, evaluate=False):
        """Selects a joint action, augmenting learned policy with a hybrid supervisor."""
        actions = []
        with torch.no_grad():
            for i in range(self.num_agents):
                # Hybrid Rule-Based Supervisor re-enabled for training
                ...
                # Supervisor is only active during training, not evaluation
                if not evaluate and min_dist < self.env.collision_radius * 1.5:
                ...
                elif not evaluate and time_left < 2.0:
    ...
    def train(self):
        for episode in range(self.num_episodes):
            ...
            num_successful = sum(1 for i in range(self.num_agents) if np.linalg.norm(self.env.positions[i] - self.env.goals[i]) < self.env.goal_radius)
